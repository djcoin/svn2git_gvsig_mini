...
public class TweetMyLocationFunc extends Functionality {
	...
	public boolean execute() {
		try {
			...
			try {
				user = Settings.getInstance().getStringValue(
						getMap().getText(R.string.settings_key_twitter_user)
								.toString());
				pass = Settings.getInstance().getStringValue(
						getMap().getText(R.string.settings_key_twitter_pass)
								.toString());
			} catch (Exception e) {
				
			}

			if (user != null && pass != null && user.trim().compareTo("") != 0
					&& pass.trim().compareTo("") != 0) {
				this.sendMyLocationTweet(user, pass);
			} else {
				getMap().getMapHandler()
						.sendEmptyMessage(Map.SHOW_TWEET_DIALOG_SETTINGS);
			}
		} catch (Exception e) {
	...
	private void sendMyLocationTweet(String user, String pass) {
		try {
			...
			twitter.setStatus(this.buildTweet(), String.valueOf(lonlat[1]),
	...
	public String buildTweet() {
		try {
			...
			return new StringBuffer().append(
					...
					" : http://www.opentouchmap.org/?lat=").append(latDouble)
					.append("&lon=").append(lonDouble).append("&zoom=").append(
							zoomin).append(" ").append(
							getMap().getResources().getString(R.string.lat))
					.append(":").append(latDouble).append(" ").append(
							getMap().getResources().getString(R.string.lon))
